🎯 Mission - 03
Module 09 — Fundamentals of Web Application with Nodejs
📚 Topics Covered
| No. | Topic |
| --- | ------ |
| 01 | 🔏 **How the web works** |
| 02 | 🔏 **Frontend, Backend Development & Scaling Ideas** |
| 03 | 🔏 **coming soon...** |
| 04 | 🔏 **coming soon...** |
| 05 | 🔏 **coming soon...** |
| 06 | 🔏 **coming soon...** |
| 07 | 🔏 **coming soon...** |
| 08 | 🔏 **coming soon...** |
| 09 | 🔏 **coming soon...** |
| 10 | 🔏 **coming soon...** |
| 11 | 🔏 **coming soon...** |
| 12 | 🔏 **coming soon...** |
| 13 | 🔏 **coming soon...** |

Module 10 — Deep Dive into Node.js Core Modules
📚 Topics Covered
| No. | Topic |
| --- | ------ |
| 01 | 🔏 **Reading Files with the fs Module** |
| 02 | 🔏 **Writing and Appending Files with the fs Module...** |
| 03 | 🔏 **Deleting Files with the fs Module...** |
| 04 | 🔏 **Introduction to the path Module...** |
| 05 | 🔏 **Building a Simple File Organizer CLI part-1...** |
| 06 | 🔏 **Building a Simple File Organizer CLI part-2...** |
| 07 | 🔏 **Introduction to the os Module...** |
| 08 | 🔏 **Hashing Data with the crypto Module...** |
| 09 | 🔏 **Encrypting and Decrypting Data with the crypto...** |
| 10 | 🔏 **Using dotenv with Configuration...** |
| 11 | 🔏 **Module Summary...** |

Module 11 — Building Modular APIs with Express.js
📚 Topics Covered
| No. | Topic |
| --- | ------ |
| 01 | 🔏 **Working with Environment-Based Configuration** |
| 02 | 🔏 **API Routes & the Difference Between GET and POST** |
| 03 | 🔏 **API Routes & the Difference Between GET and POST** |
| 04 | 🔏 **Creating a Custom Router** |
| 05 | 🔏 **Using Our Custom Route Handler** |
| 06 | 🔏 **Implementing sendJson & Cleaning Up Server.ts** |
| 07 | 🔏 **Creating parseBody & Building a Clean POST API** |
| 08 | 🔏 **Setting Up a Fake JSON Database** |
| 09 | 🔏 **Handling Dynamic Routes** |
| 10 | 🔏 **Dynamic Route Handling for PUT Requests** |
| 11 | 🔏 **Module Summary...** |

Module 12 — CRUD Operations with Express, TypeScript & PostgreSQL
📚 Topics Covered
| No. | Topic |
| --- | ------ |
| 01 | 🔏 **Create a simple server with express.js & typescript .mp4** |
| 02 | 🔏 **Installing Postgres as database** |
| 03 | 🔏 **Connect with database using pool** |
| 04 | 🔏 **Solve the previous error, add .env & connect with GitHub** |
| 05 | 🔏 **Create our first User using POST** |
| 06 | 🔏 **Getting All Users & single user using params.** |
| 07 | 🔏 **Update with PUT Method & Delete with DELETE Method.** |
| 08 | 🔏 **Delete API & Todo Post** |
| 09 | 🔏 **Not Found Route & Middleware.** |
| 10 | 🔏 **Module Summary.** |

Module 13 — Fundamentals of Web Application with Nodejs
📚 Topics Covered
| No. | Topic |
| --- | ------ |
| 01 | 🔏 **Module Introduction** |
| 02 | 🔏 **Why Configuration Is Important?** |
| 03 | 🔏 **Cleaning server.ts with user.routes.ts.** |
| 04 | 🔏 **Implementing Business Logic in user.service.ts.** |
| 05 | 🔏 **Rearranging the User Module** |
| 06 | 🔏 **Understanding Authentication & Authorization.** |
| 07 | 🔏 **Creating Password Field & Hashing Passwords.** |
| 08 | 🔏 **Bcrypt Comparison & Introduction to JWT.** |
| 09 | 🔏 **JWT Theory & Token Generation** |
| 10 | 🔏 **Higher-Order Functions & Auth Middleware** |
| 11 | 🔏 **Auth Middleware & Declaring User in a Namespace.** |
| 12 | 🔏 **Issue Solving & Role-Based Authentication** |
| 13 | 🔏 **Reviewing Our Work So Far** |
